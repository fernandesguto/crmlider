                    ...
                    <div>
                        {agency?.phone && (
                            <a 
                                ...
                                className="bg-green-600 hover:bg-green-700 text-white px-4 md:px-5 py-2 md:py-2.5 rounded-full text-xs md:text-sm font-black flex items-center shadow-lg shadow-green-500/20 transition-all transform hover:scale-105"
                            >
                                <MessageCircle size={16} className="mr-1.5 md:mr-2" />
                                WhatsApp
                            </a>
                        )}
                    </div>
                                    ...
                                    <div className="space-y-10">
                                        <div className="relative group overflow-hidden rounded-2xl bg-slate-100 aspect-video md:aspect-[16/9]">
                                            ...
                                            {selectedProperty.images && selectedProperty.images.length > 1 && (
                                                <>
                                                    ...
                                                    <button onClick={nextImage} className="absolute right-4 top-1/2 -translate-y-1/2 bg-white/20 hover:bg-white/40 text-white p-3 rounded-full backdrop-blur-md transition z-20"><ChevronRight size={28}/></button>
                                        ...
                                        <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-none">
                                            {selectedProperty.images?.map((img, idx) => (
                                                <button key={idx} onClick={() => setCurrentImageIndex(idx)} className={`w-20 md:w-24 aspect-square rounded-lg overflow-hidden flex-shrink-0 border-2 transition ${currentImageIndex === idx ? 'border-blue-600 scale-95' : 'border-transparent opacity-60'}`}>
                    ...
                    <div>
                        <div className="flex items-center space-x-2 text-slate-900 mb-6">
                            {agency?.logoUrl ? (
                                ...
                                <div className="flex items-center space-x-2 text-slate-800">
                                    <Building2 size={28} />
                                    <span className="text-xl font-bold tracking-tight">{agency?.name || 'ImobERP'}</span>
                                </div>
                            )}
                        </div>
